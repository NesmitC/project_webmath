...
def generate_recommendations(weak_themes):
    ...
    if 'уравнения_и_неравенства' in weak_themes:
        recs.append("Пройди модуль «Орфоэпия» и реши 5 тренировочных тестов.")
    if 'геометрия' in weak_themes:
        recs.append("Повтори корни с чередованием — выполни 3 упражнения из модуля «Орфография».")
    if 'текстовые_задачи' in weak_themes:
        recs.append("Потренируй упражнения на определение причастий и деепричастий — они часто встречаются на экзамене.")
    if 'сочинение' in weak_themes:
        recs.append("Напиши сочинение и отправь на проверку — я дам обратную связь.")
    if 'функции_и_графики' in weak_themes:
        recs.append("Посмотри видеоурок по выделению вводных слов и выполни интерактивное задание.")
    if not weak_themes:
        recs.append("Отличная работа! Ты готов к финальному тесту. Хочешь пройти пробный экзамен?")
